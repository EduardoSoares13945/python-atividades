...
carro_escolhido = input('Qual o tipo do carro, Popular ou Luxo? [P/L] ')
km = int(input('Quantos Km foram percorridos? '))

if carro_escolhido.upper() == 'P':
    if km <= 100:
        preco = popular + km * 0.20
    else:
        preco = popular + km * 0.10
    print(f'O valor do seu aluguel é de R${preco:.2f}')

elif carro_escolhido.upper() == 'L':
    if km <= 200:
        preco = luxo + km * 0.30
    else:
        preco = luxo + km * 0.25
